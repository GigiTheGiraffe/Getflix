...
try {
    ...
    if ($genre && !$sort) {
        $stmt = $conn->prepare("SELECT poster_path, title FROM movies_list  WHERE genre_1 = :genre OR genre_2 = :genre OR genre_3 = :genre ORDER BY id LIMIT 20 OFFSET :offset");
        $stmt->bindParam(':genre', $genre, PDO::PARAM_STR);
    } elseif ($sort && !$genre) {
        $stmt = $conn->prepare("SELECT poster_path, title, genre_1, release_date FROM movies_list ORDER BY release_date $sort LIMIT 20 OFFSET :offset");
    } elseif ($sort && $genre) {
        $stmt = $conn->prepare("SELECT poster_path, title FROM movies_list  WHERE genre_1 = :genre OR genre_2 = :genre OR genre_3 = :genre ORDER BY release_date $sort LIMIT 20 OFFSET :offset");
        $stmt->bindParam(':genre', $genre, PDO::PARAM_STR);
    } else {
        $stmt = $conn->prepare("SELECT poster_path, title, genre_1 FROM movies_list ORDER BY id LIMIT 20 OFFSET :offset");
    }
