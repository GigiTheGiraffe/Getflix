...
async function loadMovies() {
    ...
    if(countError !== 0) {
        if(alertNumber > 0) return;
        alertNumber++;
        alert('Il y a une erreur avec le chargement. Veuiller recharger la page. Nous nous excusons du dérangement.')
        return; 
    }
    loading = true;
            ...
            if (!displayedMovieTitle.has(film.title)) {
                ...
                movieItem.innerHTML = `
                    ...
                    <div class="text-container">
                        <h5 class="texte left-align">${film.title}</h5>
                        <h5 class="texte right-align">${film.genre_1}</h5>
                    </div>
                `;
